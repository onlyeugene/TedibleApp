...
const DesktopMenu: React.FC<DesktopMenuProps> = ({ path }) => {
  ...
  const { dropdown, handleDropdown, dropdownRef } = useDropdownExternal();
    ...
    return (
      <>
        ...
        <div className="sm:flex gap-4 items-center hidden">
          {user?.image ? (
            <Image src={user?.image as string} alt="user image" width={25} height={25} className="rounded-full" priority />
          ) : (
        ...
        {dropdown && (
          <ul ref={dropdownRef} className="absolute border bg-white w-[8rem] right-4 top-11 shadow-md py-2 rounded-md text-center text-xs">
  ...
  return (
    <>
      ...
      <div className="sm:flex items-center gap-5 hidden">
        <Link href="/login">
          <Button className="border-[#FF7834] py-2 px-6 rounded-md text-[#FF7834]">Login</Button>
        </Link>
        <Link href="/register">
          <Button className="bg-[#FF7834] py-2 px-6 rounded-md border-[#FF7834] text-white">Register</Button>
        </Link>
      </div>
    </>
  );
